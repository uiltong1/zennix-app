...
export default {
  ...
  data () {
    return {
      ...
      data: {
        columns: [
          { label: 'Nome', field: 'no_seguradora' },
          { label: 'Sigla', field: 'sg_seguradora' },
  ...
  methods: {
    ...
    toggle (data) {
      this.$q.dialog({
        // title: 'Al',
        message: 'Deseja alterar o status da seguradora?',
        cancel: true,
        persistent: true
      }).onOk(() => {
        SeguradorasServices.toggle(data)
          .then(result => {
